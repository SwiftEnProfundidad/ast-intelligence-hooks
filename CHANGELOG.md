...
All notable changes to `pumuki-ast-hooks` will be documented in this file.
...
- **Paths with spaces support**: `evidence-guard` wrapper now resolves `PROJECT_ROOT` from `cwd` by searching for `package.json` upwards, fixing daemon startup in paths like `/Volumes/Backup Plus/...`.
- **Quoted npm scripts**: `addNpmScripts` now wraps paths in quotes to prevent shell word-splitting on paths with spaces.
...
- **God class detector loop**: Fixed premature exit when encountering ValueObject/DTO/Entity classes. The detector now correctly continues analyzing all classes in a file instead of stopping after the first skipped class.
...
### Fixed
- **DIP detector for iOS**: protocol-like types are detected even with trailing punctuation (e.g. `LoginUseCase,`).

## [6.0.12] - 2026-01-13

### Fixed
- **DIP detector for iOS**: protocol-like types are detected even with composition markers (e.g. `LoginUseCase & Sendable`).

## [6.0.11] - 2026-01-13

### Fixed
- **DIP detector for iOS**: protocol-like types are detected even when generics are present (e.g. `RegisterUseCase<Auth>`).

## [6.0.10] - 2026-01-13

### Fixed
- **DIP detector for iOS**: protocol-like types are detected even with module prefixes or optional markers (e.g. `Domain.LoginUseCase?`).

## [6.0.9] - 2026-01-13

### Fixed
- **DIP detector for iOS**: protocol-named types like `LoginUseCase` no longer report as concrete dependencies unless they are `*Impl`.
