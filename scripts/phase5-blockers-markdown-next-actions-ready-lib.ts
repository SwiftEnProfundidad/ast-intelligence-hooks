export const appendPhase5BlockersReadyNextActions = (params: {
  ...
  if (!params.hasAdapterReport) {
    params.lines.push(
      '- Optional: generate Adapter report for adapter diagnostics traceability (`npm run toolkit:adapter-real-session-report -- --status-report .audit-reports/adapter/adapter-session-status.md --out .audit-reports/adapter/adapter-real-session-report.md`).'
    );
  }
  params.lines.push('');
};
