...
import { buildConsumerSupportBundleMarkdown } from './consumer-startup-failure-support-bundle-lib';
...
export const buildConsumerSupportBundleReport = (
  ...
  return buildConsumerSupportBundleMarkdown({
    generatedAtIso: new Date().toISOString(),
    options,
    authStatus,
    repoInfo: repoInfoResult.ok ? repoInfoResult.data : undefined,
    actionsPermissions: actionsPermissionsResult.ok ? actionsPermissionsResult.data : undefined,
    actionsPermissionsError: actionsPermissionsResult.ok ? undefined : actionsPermissionsResult.error,
...
export const writeConsumerSupportBundleReport = (params: {
  markdown: string;
  outFile: string;
}): string => {
  const outputPath = resolve(process.cwd(), params.outFile);
