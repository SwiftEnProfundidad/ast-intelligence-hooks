...
class NotificationCenterService {
  constructor(config = {}) {
    ...
    this.logPath = config.logPath || path.join(this.repoRoot, '.audit-reports', 'notifications.log');
  ...
  log(level, message, data = {}) {
    ...
    try {
      ...
      console.error('[NotificationCenter] Log write failed:', error.message);
    }

    // Console output para debugging
    if (level === 'error') {
      console.error(`[NotificationCenter] ${message}`, data);
    } else if (level === 'warn') {
      console.warn(`[NotificationCenter] ${message}`, data);
    }
  }
