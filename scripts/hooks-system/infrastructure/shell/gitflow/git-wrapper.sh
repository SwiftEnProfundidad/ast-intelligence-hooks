...
validate_gitflow_content() {
  ...
  local has_android=$(echo "$commit_messages" | grep -ciE "android|kotlin|detekt|ast.*rule|compose" || echo "0")
  local has_mcp=$(echo "$commit_messages" | grep -ciE "mcp|server.*notification|cursor.*integration" || echo "0")
  local has_watchdog=$(echo "$commit_messages" | grep -ciE "watchdog|monitor|timestamp.*parsing" || echo "0")
  local has_gitflow=$(echo "$commit_messages" | grep -ciE "git.*flow|enforcer|wrapper|branch" || echo "0")
  local has_hooks=$(echo "$commit_messages" | grep -ciE "hook|pre-commit|session.*loader|evidence" || echo "0")
  local has_frontend=$(echo "$commit_messages" | grep -ciE "frontend|react|next|dashboard" || echo "0")
  local has_backend=$(echo "$commit_messages" | grep -ciE "backend|nestjs|api|controller" || echo "0")
