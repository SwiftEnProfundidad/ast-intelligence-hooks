...
|-----------|-----------------|
| Este plan | [🚧] - `PARITY-IOS-SWIFTDATA-001` / retomar la paridad iOS/SwiftData tras cerrar el bug externo `PUMUKI-INC-128`. Cierre externo confirmado en `pumuki@6.3.172`: `PRE_WRITE` usa `scope=staged` cuando hay staged soportado y transforma la deuda global ajena en advisory no bloqueante para audits acotados. |
...
- Estado de backlog externo tras revalidación 2026-05-12: `R_GO` queda actualizado con `PUMUKI-INC-128` cerrado en `pumuki@6.3.172`; `SAAS`, `R_GO` y `Flux_training` no muestran activos. La prioridad operativa vuelve al backlog interno permitido, empezando por `PARITY-IOS-SWIFTDATA-001`.
...
- Regresión local Pumuki: `npx tsx --test integrations/lifecycle/__tests__/audit.test.ts` devuelve `4/4 pass`; `git diff --check` limpio; `npm pack --dry-run --silent` genera `pumuki-6.3.172.tgz`.
- Replay consumer aislado en `/tmp/stack-my-architecture__pumuki-inc-128-replay`: `npx --yes --package pumuki@6.3.172 pumuki audit --stage=PRE_WRITE --json` devuelve `exit=0`, `scope.kind=staged`, `staged_matching_extensions_count=1`, `files_scanned=1`, `blocking_findings_count=0` y un único advisory no bloqueante `AUDIT_SCOPED_GLOBAL_ENFORCEMENT_ADVISORY`.
