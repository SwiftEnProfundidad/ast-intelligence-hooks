...
|-----------|-----------------|
| Este plan | `[🚧] - PUMUKI-INC-060 / consumers: cierre de repin 6.3.140 y bug PRE_PUSH de ramas actualizadas desde base. Estado real 2026-05-05: `pumuki@6.3.140` queda publicado y mergeado en Pumuki; RuralGo queda repineado primero y mergeado en `develop` vía PR #1906; Flux PR #13 y SAAS PR #16 quedan abiertos con ramas repineadas a `6.3.140`, pero el push de resolución contra `origin/main` detecta un bug nuevo de Pumuki: `GIT_ATOMICITY_COMMIT_MESSAGE_TRACEABILITY` audita commits merge heredados de `main` (`Merge pull request #12/#15`) como si fueran commits propios del rollout. Fix en curso: `PRE_PUSH` debe filtrar commits alcanzables desde ramas base (`main/develop/origin/*/upstream/*`) y saltar commits merge, manteniendo el bloqueo sobre commits propios no trazables; regresión local añadida en `integrations/git/__tests__/gitAtomicity.test.ts` y suite dirigida `207/207 pass`. Pendiente: commitear/publicar patch siguiente, repinear primero RuralGo y después reintentar push/merge de Flux y SAAS sin bypass.` |
