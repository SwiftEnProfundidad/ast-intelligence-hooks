...
- [🚧] - `PARITY-IOS-001` / `skills.ios.no-callback-style` / detector AST real para callbacks Swift legacy: continuar paridad iOS con nodos accionables y fix hacia `async/await` o bridges explícitos.
...
- Estado: ✅ PUMUKI-INC-143 cerrado en `pumuki@6.3.252`; sin bugs externos abiertos conocidos tras el repin RuralGo. Task iOS actual: ✅ `skills.ios.no-anyview` preparado para `pumuki@6.3.263`; siguiente 🚧 `skills.ios.no-callback-style`.
...
Último avance operativo: `[✅] - PARITY-IOS-001 / skills.ios.no-anyview` cierra la slice SwiftUI con evidencia AST-style para `AnyView` en Swift production. Quedan enlazados detector textual de líneas, extractor heurístico, payload de `primary_node`/`related_nodes`, tests dirigidos, changelog y notas de release. Evidencia local: tests dirigidos `121/121 pass`; `npm run -s typecheck` -> OK; `npm run -s skills:lock:check` -> FRESH; `git diff --check` -> limpio; `npm pack --dry-run --silent` -> `pumuki-6.3.263.tgz`; `npm run -s validation:package-smoke` -> OK; `npm test -- --runInBand` -> OK (`4/4` suites, `23/23` tests).
