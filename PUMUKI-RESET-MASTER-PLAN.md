...
Tarea activa única: `[🚧] - PUMUKI-INC-087 — Unificar el contrato de acción/remediación visible` con `084` ya cerrada en la línea publicada y en RuralGo, `086` ya cerrada en la línea publicada y `089` todavía pendiente porque la remediación sugerida sigue apuntando a `pumuki@latest`. El objetivo inmediato pasa a ser eliminar la ambigüedad entre `action=ask` y `next_action.kind=run_command` para que el runtime exponga una única decisión accionable y reproducible antes de abrir el siguiente frente externo.
