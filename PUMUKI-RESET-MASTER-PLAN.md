...
Último avance operativo: `[✅] - PR #890 / paridad local de CI recuperada` corrige la deriva entre la suite local y los contratos actuales de evidence/MCP/package smoke. GitHub Actions no ejecuta pasos reales porque las anotaciones de los check-runs devuelven `The job was not started because your account is locked due to a billing issue`; Snyk queda bloqueado aparte por cuota de tests privados. La rama queda validada localmente con la barra equivalente: `npm run -s typecheck`, `npm run -s test:heuristics`, `npm run -s test:deterministic`, `npm run -s validation:package-smoke`, `npm run -s validation:package-smoke:minimal`, `npm run -s validation:tracking-single-active`, `node --import tsx scripts/check-package-manifest.ts`, `node --import tsx --test scripts/__tests__/build-mock-consumer-ab-report.test.ts` y `npm run -s validation:phase5-execution-closure -- --repo mock/consumer --out-dir .audit-reports/phase5 --mock-consumer`. Cierre técnico: los tests de evidence esperan ya `auto_runtime_coverage_ratio`, `semantic_enforcement_ratio` y `global_skills_enforcement`; `auto_execute_ai_start` refleja el fail-closed actual de skills incompletas; el bridge stdio no cierra un `staleServer` inexistente; package smoke acepta bloqueos pre-gate de Git atomicity sin copiar evidencia obsoleta; y el informe mock consumer A/B distingue evidencia BLOCK v2.1 de bloqueo pre-gate sano.
