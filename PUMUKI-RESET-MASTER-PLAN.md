...
Tarea activa única: `[🚧] - PUMUKI-INC-081 / PUMUKI-INC-083 — Separar blocker real de warning PRE_WRITE y hacer accionable el tracking canónico` el backlog externo de RuralGo vuelve a ser prioritario y deja invalidada cualquier task interna no alineada: `079/080` quedan tratadas como stale ya corregidas en la línea publicada, `082/088` apuntan a deuda consumer-only y el primer paquete upstream útil pasa a ser `081 + 083`. El objetivo inmediato es que `PRE_WRITE` deje de mezclar `TRACKING_CANONICAL_IN_PROGRESS_INVALID` con `EVIDENCE_PREWRITE_WORKTREE_WARN` al mismo nivel y que la salida nombre las entradas `🚧` concretas que disparan el bloqueo, sin obligar a grep manual en el MD canónico.
