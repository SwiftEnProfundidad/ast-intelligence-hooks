...
Último avance operativo: `[✅] - PUMUKI-INC-121 recuperar menú interactivo legacy real` recupera la shell interactiva del framework menu con estado visible, jerarquía de secciones y launcher TS estable sin depender del pipe IPC de `tsx/cli` en tests. Evidencia local: suite dirigida `11/11 pass` en la superficie del menú (`framework-menu-consumer-runtime-menu`, `framework-menu-layout-resolve`, `framework-menu-cli-runtime`, `framework-menu-ui-components-render`); la invocación de CLI directa con `node --import tsx scripts/framework-menu.cli.ts` devuelve exit 0 en clásico y moderno.
...
- `[✅] - PUMUKI-INC-121 recuperar menú interactivo legacy real`: el menú actual no cumple la experiencia legacy pedida; debe recuperar navegación interactiva clara, jerarquía visual útil, opciones primarias comprensibles y paridad funcional sin degradar el flujo enterprise.
