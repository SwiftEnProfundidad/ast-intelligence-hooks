...
Último avance operativo: `[🚧] - PARITY-IOS-001 / Logging production - print/logs ad-hoc y PII` retoma prioridad iOS tras comprobar que SAAS, RuralGo y Flux no tienen bugs Pumuki externos activos. Slice en curso: detector AST/textual para `print`, `debugPrint`, `dump`, `NSLog`, `os_log` y logging de tokens/credenciales/emails/IDs sensibles en Swift production, enlazado en extractor, preset, registry y markdown.
