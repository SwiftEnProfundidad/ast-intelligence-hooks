...
|-----------|-----------------|
| Este plan | `[🚧] - PUMUKI-INC-059 / RuralGo: reproducir y cerrar gap enforcement skills hard OCP/SRP/DIP/ISP/LSP en PRE_WRITE/PRE_COMMIT/PRE_PUSH. Estado 2026-05-05: reproducida la causa raíz en Pumuki: las heurísticas iOS SOLID existían (`heuristics.ios.solid.*`), pero el runtime solo forzaba extracción AST cuando el lock compilado aportaba reglas AUTO mapeadas; el lock iOS traía `skills.ios.guideline.ios.verificar-que-no-viole-solid-srp-ocp-lsp-isp-dip` y no existía alias canónico/mapeo `skills.ios.no-solid-violations`, por lo que PRE_WRITE podía no pedir heurísticas y dejar pasar OCP/SRP. Fix en `bugfix/pumuki-inc-059-ios-solid-prewrite`: normalización markdown iOS SOLID -> `skills.ios.no-solid-violations`, alias legacy del lock real, mapping a OCP/SRP/DIP/ISP/LSP, promoción bloqueante desde PRE_WRITE/PRE_COMMIT/PRE_PUSH/CI y preset iOS SOLID adelantado a PRE_WRITE. Evidencia local: 242 tests verdes en detectores iOS/extractHeuristicFacts/ruleset/gate; regresión nueva `evaluatePlatformGateFindings bloquea OCP iOS SOLID en PRE_WRITE sin flag experimental de heuristicas`. Siguiente paso obligatorio: reproducir con consumer/worktree aislado o tarball local, preparar release, publicar y repinear primero RuralGo.` |
