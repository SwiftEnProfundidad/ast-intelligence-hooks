...
Último cierre operativo: `[✅] - Slice S2.c — Extender el smoke contractual de governance a los hooks PRE_WRITE, PRE_COMMIT y PRE_PUSH` el smoke contractual cruzado deja de cubrir solo menú, `status`, `doctor`, `pre_flight_check` y `ai_gate_check` y pasa a congelar también el contrato visible actual de hooks: `PRE_WRITE` queda cubierto a través de `runLifecycleCli(['sdd','validate','--stage=PRE_WRITE'])` y `PRE_COMMIT` / `PRE_PUSH` quedan cubiertos a través de sus stage runners reales, manteniendo verde la suite dirigida `scripts/__tests__/governance-console-cross-surface-smoke.test.ts` en `refactor/s2c-governance-hook-smoke`.

Tarea activa única: `[🚧] - PUMUKI-INC-084 / PUMUKI-INC-086 / PUMUKI-INC-089 — Hacer canónico el contrato bloqueado, alinear stage y fijar remediación reproducible` con `081/083` ya cerradas en la línea publicada y en RuralGo, el siguiente write set útil del backlog externo pasa a ser `084 + 086 + 089`: `status`/`doctor` siguen pudiendo dejar `issues=[]` cuando el runtime bloquea, la salida mezcla `governanceNextAction.stage=PRE_WRITE` con snapshots `PRE_PUSH`, y la remediación recomendada sigue apuntando a `pumuki@latest` en vez de al runtime diagnosticado. El objetivo inmediato es cerrar esa incoherencia de contrato antes de abrir otro frente.
