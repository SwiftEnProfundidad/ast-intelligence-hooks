...
- [🚧] - `PARITY-IOS-001` / `skills.ios.no-dispatchqueue` / detector AST real para GCD Swift legacy: continuar paridad iOS con nodos accionables y fix hacia structured concurrency.
...
- Estado: ✅ PUMUKI-INC-143 cerrado en `pumuki@6.3.252`; sin bugs externos abiertos conocidos tras el repin RuralGo. Task iOS actual: ✅ `skills.ios.no-callback-style` preparado para `pumuki@6.3.264`; siguiente 🚧 `skills.ios.no-dispatchqueue`.
...
Último avance operativo: `[✅] - PARITY-IOS-001 / skills.ios.no-callback-style` cierra la slice de callbacks Swift legacy con evidencia AST-style para firmas `completion`/`handler` `@escaping` fuera de bridges aprobados. Quedan enlazados detector textual de líneas, extractor heurístico, payload de `primary_node`/`related_nodes`, tests dirigidos, changelog y notas de release. Evidencia local: tests dirigidos `121/121 pass`; `npm run -s typecheck` -> OK; `npm run -s skills:lock:check` -> FRESH; `git diff --check` -> limpio; `npm pack --dry-run --silent` -> `pumuki-6.3.264.tgz`; `npm run -s validation:package-smoke` -> OK; `npm test -- --runInBand` -> OK (`1549 pass`, `4 skipped`, Jest `4/4` suites, `23/23` tests).
