...
|-----------|-----------------|
| Este plan | `[✅] - RG-P0-1 cerrado.` Slice 1 cerrada: el adapter MCP ya activa la superficie enterprise por defecto. Slice 2 legacy cerrada: el `Chevrolet` vuelve a instalar, bloquear `pre-commit` y completar `wrap-up`/`intent` en worktrees modernos; además el ruido residual de `rules/evidence` y el warning de `skill files installation` ya quedan resueltos. Slice 3 cerrada: el legacy ya escribe `.pumuki/adapter.json` como capa agnóstica opcional, añade ese artefacto al bloque gestionado de `.git/info/exclude` y el replay real del adapter ya ejecuta semántica útil (`pre_commit` bloquea, `pre_push` valida GitFlow y `mcp.automation` devuelve sesión MCP verificable). Ajuste adicional ya cerrado: el `pre-commit` legacy ahora bloquea también por `MEDIUM` y `LOW`. Decisión vigente: no aceptar un segundo transplante mínimo hasta que aparezca una pieza claramente aditiva y con runtime real. Criterio hard nuevo: cualquier futura incorporación desde `freezer-current` al legado tendrá que entrar por motor AST/node-intelligence real (`astNodeIds` + heurísticas dinámicas compiladas desde skills/reglas), priorizando Swift y el resto de lenguajes modernos; no se aceptan reglas hardcodeadas estáticas ni simples `.mdc` como forma final de enforcement. Primer candidato identificado: `ios-swiftui-expert-guidelines` / `ios-swiftui-modernization-v2`. Las treinta y cinco primeras traducciones reales ya cerradas con detector AST operativo: `skills.ios.no-navigation-view` pasa al detector AST iOS legacy y se elimina su regex estática; `skills.ios.no-sheet-is-presented` ya se detecta por AST dentro de `View` sin introducir enforcement estático nuevo; `skills.ios.no-legacy-onchange` ya se detecta por AST al localizar el overload legado de un solo parámetro; `skills.ios.no-foreground-color` ya se detecta por AST al localizar `foregroundColor` dentro de `View`, preservando `foregroundStyle` como caso limpio; `skills.ios.no-corner-radius` ya se detecta por AST al localizar `cornerRadius` dentro de `View`, preservando `clipShape(.rect(cornerRadius:))` como caso limpio; `skills.ios.no-tab-item` ya se detecta por AST al localizar `tabItem` dentro de `View`, preservando la nueva `Tab` API como caso limpio; `skills.ios.no-scrollview-shows-indicators` ya se detecta por AST al localizar `ScrollView(..., showsIndicators: ...)` dentro de `View`, preservando `.scrollIndicators(.hidden)` como caso limpio; `skills.ios.no-observable-object` ya se detecta por AST al localizar clases que heredan `ObservableObject`, preservando `@Observable` como caso limpio; y `skills.ios.no-legacy-swiftui-observable-wrapper` ya se detecta por AST al localizar `@StateObject` o `@ObservedObject` dentro de structs `View`, preservando `@State` y `@Bindable` como casos limpios; y `skills.ios.no-passed-value-state-wrapper` ya se detecta por AST al localizar `@State` o `@StateObject` reenvueltos desde parámetros del `init`, preservando `@Binding` y el estado local propio como casos limpios; y `skills.ios.no-on-tap-gesture` ya se detecta por AST al localizar `onTapGesture` dentro de un `View`, preservando `Button` como caso limpio; y `skills.ios.no-string-format` ya se detecta por AST al localizar `String(format:)` dentro de un `View`, preservando la interpolación Swift como caso limpio; y `skills.ios.no-foreach-indices` ya se detecta por AST al localizar `ForEach(...indices...)` dentro de un `View`, preservando `ForEach(items, id: \\.id)` como caso limpio; y `skills.ios.no-contains-user-filter` ya se detecta por AST al localizar flujos `filter { ... contains(query) }` dentro de un `View`, preservando `localizedStandardContains(query)` como caso limpio; y `skills.ios.no-geometryreader` ya se detecta por AST al localizar `GeometryReader` dentro de un `View`, preservando `containerRelativeFrame()` como caso limpio; y `skills.ios.no-font-weight-bold` ya se detecta por AST al localizar `fontWeight(.bold)` dentro de un `View`, preservando `bold()` como caso limpio. Nota de proceso: la desviación TDD quedó acotada a `skills.ios.no-foreach-indices`; la slice `skills.ios.no-font-weight-bold` sí volvió a rojo→verde puro. La consolidación de `skills.ios.no-uiscreen-main-bounds` queda ya cerrada, `skills.ios.prefer-swift-testing` ya se detecta por AST al localizar suites XCTest-only con `import XCTest`, `skills.ios.no-xctassert` ya se detecta por AST al localizar `XCTAssert*` y `XCTFail` dentro de suites modernas con `import Testing` o `@Test`, y `skills.ios.no-xctunwrap` ya se detecta por AST al localizar `XCTUnwrap` dentro de suites modernas con `import Testing` o `@Test`, preservando `#require` como caso limpio, `skills.ios.no-wait-for-expectations` ya se detecta por AST al localizar `wait(for:)` y `waitForExpectations(timeout:)` dentro de tests XCTest asíncronos, preservando `await fulfillment(of:)` como caso limpio, y `skills.ios.no-legacy-expectation-description` ya se detecta por AST al localizar `expectation(description:)` sin flujo moderno de `fulfillment(of:)` o `confirmation(...)`, preservando `await fulfillment(of:)` como caso limpio, y `skills.ios.no-mixed-testing-frameworks` ya se detecta por AST al localizar convivencia simultánea de `XCTestCase` con `import Testing` o `@Test` en el mismo fichero. Con esto queda cerrado `ios-swiftui-expert-guidelines`; además, `skills.ios.no-nsmanagedobject-boundary` ya se detecta por AST al localizar `NSManagedObject` en fronteras compartidas de propiedades y parámetros, preservando `NSManagedObjectID` como caso limpio, y `skills.ios.no-nsmanagedobject-async-boundary` ya se detecta por AST al localizar firmas `async` que aceptan o devuelven `NSManagedObject`, preservando `NSManagedObjectID` como caso limpio, y `skills.ios.no-core-data-layer-leak` ya se detecta por AST al localizar orquestación Core Data fuera de capas `Repository`/`Store`/`Persistence`, preservando la capa repositorio como caso limpio, y `skills.ios.no-nsmanagedobject-state-leak` ya se detecta por AST al localizar `NSManagedObject` almacenado dentro de contenedores de estado tipo `ViewModel` o `State`, preservando `NSManagedObjectID` como caso limpio. Con esto queda cerrado `ios-core-data-guidelines`; además, `skills.backend.no-empty-catch` ya se detecta por AST en `apps/backend/**` al localizar `catch` vacíos en runtime backend legacy, y `skills.backend.no-console-log` ya se detecta por AST al localizar `console.log(...)` en runtime backend legacy, preservando `this.logger.log(...)` como caso limpio, y `skills.backend.avoid-explicit-any` ya se detecta por AST al localizar anotaciones explícitas `: any` en runtime backend legacy, preservando `unknown` como caso limpio, y `skills.backend.no-solid-violations` ya se detecta por AST al localizar clases backend con exceso de dependencias inyectadas en constructor, preservando servicios con dependencias enfocadas como caso limpio, y `skills.backend.no-god-classes` ya se detecta por AST al localizar clases backend con exceso simultaneo de propiedades y metodos, preservando clases enfocadas como caso limpio. Con esto queda cerrado `backend-guidelines`; además, `skills.frontend.no-console-log` ya se detecta por AST al localizar `console.log(...)` en runtime frontend legacy, preservando `logger.log(...)` como caso limpio, y `skills.frontend.no-empty-catch` ya se detecta por AST al localizar `catch` vacíos en runtime frontend legacy, preservando manejo explícito del error como caso limpio, y `skills.frontend.avoid-explicit-any, skills.frontend.no-solid-violations, skills.frontend.enforce-clean-architecture y skills.frontend.no-god-classes` ya se detecta por AST al localizar anotaciones explícitas `: any` en runtime frontend legacy, preservando `unknown` como caso limpio. Con esto queda cerrado `frontend-guidelines`; ademas, `skills.android.no-thread-sleep`, `skills.android.no-globalscope` y `skills.android.no-runblocking` ya se detecta por AST al localizar `Thread.sleep(...)`, `GlobalScope.*` y `runBlocking` en runtime Android legacy, preservando `delay(...)`, `viewModelScope.launch()` y `coroutineScope { ... }` como casos limpios. Con esto queda cerrado `android-guidelines`. El siguiente frente canónico queda definido como `backend-guideline-foundations`, derivado de los `skills.backend.guideline.*` top-level de `skills.lock.json`; `skills.backend.guideline.backend.callback-hell-usar-async-await`, `skills.backend.guideline.backend.clean-por-feature-presentation-application-domain-infrastructure-domai`, `skills.backend.guideline.backend.controllers-delgados-solo-routing-y-validacio-n-lo-gica-en-servicios`, `skills.backend.guideline.backend.dtos-en-boundaries-validacio-n-en-entrada-salida` y `skills.backend.guideline.backend.hardcoded-values-config-en-variables-de-entorno` ya se detecta por AST al localizar callbacks anidados, imports cruzando capas backend, controllers con lógica excesiva, boundaries backend con tipos inline sin DTO y valores de configuración hardcodeados, preservando `async/await`, imports hacia `application`, controllers delgados, DTOs explícitos y lectura desde `process.env` como casos limpios. Con esto queda cerrado `backend-guideline-foundations`. La siguiente `🚧` pasa a reconciliar la línea local con la release realmente publicada: `npm view pumuki version` confirma `6.3.98`, mientras `VERSION` y `package.json` locales siguen en `6.3.81`; el parche de notificaciones ya está validado y persistido como commit `9314f24` en `release/6.3.81` (`adelante 1`), pero no existe rama ni worktree local/remoto `release/6.3.97`. El siguiente paso correcto es promocionar ese commit a una nueva línea de publicación superior a `6.3.98` (hotfix o release nueva) y solo después publicar o actualizar consumers.` |
...
Último cierre operativo: `[✅] - PUMUKI-INC-084 — Lista canónica de issues ya visible en status/doctor` el replay real en RuralGo con `pumuki@6.3.115` ya devuelve `issues` canónicos tanto en `status` como en `doctor` cuando governance queda en `attention`, así que el warning operativo deja de quedar partido entre `attention_codes` y `human_summary_preview` sin lista consumible.

Último cierre operativo: `[✅] - PUMUKI-INC-087 — Contrato de acción ya no se reproduce en la línea publicada` el replay real en RuralGo con `pumuki@6.3.115` sobre `status --json` y `doctor --json` ya no expone `governanceNextAction`, así que la ambigüedad `action=ask` vs `next_action.kind=run_command` deja de estar presente en la salida visible del runtime publicado y el backlog externo queda cerrado por no reproducción.

Tarea activa única: `[🚧] - PUMUKI-INC-088 — Hacer trazable el inventario local real de dependencias` con `084` y `087` ya cerradas en la línea publicada y en RuralGo, `086` ya cerrada en la línea publicada y `089` todavía pendiente porque la remediación sugerida sigue apuntando a `pumuki@latest`. Avance local en `bugfix/inc-088-dependency-inventory`: `status` y `doctor` mantienen `trackedNodeModulesCount`/`trackedNodeModulesPaths` como señal estricta de seguridad Git y añaden `dependencyInventory` para exponer `package.json`, lockfile, `node_modules`, declaración de `pumuki`, versión instalada y binario local. Regresión focalizada verde: `npx tsx --test integrations/lifecycle/__tests__/status.test.ts integrations/lifecycle/__tests__/doctor.test.ts` (`32/32`). Pendiente antes de cerrar: PR, publicación útil y replay/actualización de consumer si aplica; `npm run -s typecheck` sigue bloqueado por deuda base ajena a esta slice (`bootstrapManifest.ts`, `cliSdd.ts`, `governanceObservationSnapshot.ts`, `framework-menu-system-notifications-remediation.ts`).
