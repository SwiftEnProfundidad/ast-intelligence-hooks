...
|-----------|-----------------|
| Este plan | `[🚧] - PUMUKI-INC-059 / RuralGo: reproducir y cerrar gap enforcement skills hard OCP/SRP/DIP/ISP/LSP en PRE_WRITE/PRE_COMMIT/PRE_PUSH. Estado de entrada 2026-05-05: el cierre operativo anterior deja `pumuki@6.3.141` publicado en npm y mergeado en Pumuki vía PR #853; RuralGo queda repineado primero y mergeado en `develop` vía PR #1907; Flux queda repineado y mergeado en `main` vía PR #13; SAAS queda repineado y mergeado en `main` vía PR #16. El bloqueo nuevo `GIT_ATOMICITY_COMMIT_MESSAGE_TRACEABILITY` sobre commits merge heredados de `main` queda corregido filtrando commits alcanzables desde ramas base (`main/develop/origin/*/upstream/*`) y saltando commits merge, sin relajar el bloqueo sobre commits propios no trazables o no atómicos. Siguiente paso obligatorio: volver al bug externo abierto de RuralGo `PUMUKI-INC-059` y contrastar con código/tests antes de tocar nuevas slices internas.` |
