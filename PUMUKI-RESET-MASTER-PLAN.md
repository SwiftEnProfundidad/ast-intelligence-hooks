...
Tarea activa única: `[🚧] - PUMUKI-INC-083 — Hacer accionable el tracking canónico en la salida visible del runtime` con `081/082/084/085/086/089` ya cerradas en la línea publicada y en RuralGo; la salida todavía deja al consumidor sin una referencia accionable clara cuando el tracking canónico entra en conflicto, así que el objetivo inmediato es exponer las entradas `🚧` causantes en `status`/`doctor` antes de abrir otro frente.
