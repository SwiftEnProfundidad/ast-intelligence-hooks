...
Tarea activa única: `[🚧] - PUMUKI-INC-084 / PUMUKI-INC-086 / PUMUKI-INC-089 — Hacer canónico el contrato bloqueado, alinear stage y fijar remediación reproducible` con `083` ya cerrada en la línea publicada y en RuralGo; el siguiente write set útil del backlog externo pasa a ser `084 + 086 + 089`: `status`/`doctor` seguían pudiendo dejar `issues=[]` cuando el runtime bloquea, la salida mezclaba `governanceNextAction.stage=PRE_WRITE` con snapshots `PRE_PUSH`, y la remediación recomendada seguía apuntando a `pumuki@latest` en vez de al runtime diagnosticado. El objetivo inmediato es cerrar esa incoherencia de contrato antes de abrir otro frente.
