...
|-----------|-----------------|
| Este plan | `[🚧] - PUMUKI-INC-119 baseline enterprise activo por defecto`. Bug consumer RuralGo/Codex detectado al mostrar features de auditoría (`sdd`, `heuristics`, `learning_context`, `analytics`, `operational_memory`, `saas_ingestion`) en `off`, dejando a Pumuki sin auditar toda la superficie que debe gobernar. |
...
Tarea activa única: `[🚧] - PUMUKI-INC-119 baseline enterprise activo por defecto`. Criterio de cierre: todos los features de auditoría/gate declarados en `experimentalFeatures` deben salir `strict source=default blocking=true`; los mensajes `*_EXPERIMENTAL_DISABLED` solo pueden aparecer cuando el consumer apaga explícitamente el feature por env `off`; `status`, lifecycle, MCP y SDD deben tener regresión para impedir volver a `default-off`.
