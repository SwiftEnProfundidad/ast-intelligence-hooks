...
Último cierre operativo: `[✅] - Slice S2.b — Extender el smoke contractual de governance a status, doctor y ai_gate_check` el smoke contractual cruzado deja de cubrir solo menú, `status` y `pre_flight_check` y pasa a congelar también `doctor` y `ai_gate_check` sobre el mismo contrato visible de `Pre-write`, `reason` e `instruction`; el cierre queda cubierto por la suite dirigida `scripts/__tests__/governance-console-cross-surface-smoke.test.ts` en verde dentro de `refactor/s2b-governance-smoke`.

Tarea activa única: `[🚧] - Slice S2.c — Extender el smoke contractual de governance a los hooks PRE_WRITE, PRE_COMMIT y PRE_PUSH` con el backlog externo ya en `0` y `S2.b` cerrado, el siguiente hueco contractual mínimo pasa a ser congelar también la salida visible de hooks sobre el mismo bloque canónico (`Pre-write`, `reason`, `instruction`, `NEXT` y `REMEDIATION`), cerrando así la última superficie operativa del paquete mínimo de S1 sin reabrir frentes paralelos ni tocar runtime antes de que la regresión exista.
