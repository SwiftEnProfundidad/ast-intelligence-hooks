...
|-----------|-----------------|
| Este plan | [🚧] - `PUMUKI-INC-128` / RuralGo XCTest brownfield no debe bloquear por Swift Testing obligatorio. Estado 2026-05-05: bug externo activo reportado por RuralGo en `pumuki@6.3.153`; corregido localmente en detector/extractor iOS con regresión `.spec.swift`, pendiente de commit, publicación útil y repin primero en RuralGo. |
...
Último avance operativo: `[✅] - PARITY-IOS-001 / SwiftUI ForEach filtering + Color static member lookup` convierte dos reglas declarativas de `swiftui-expert-skill` en cobertura AST por nodos: `skills.ios.guideline.ios-swiftui-expert.avoid-inline-filtering-in-foreach-prefilter-and-cache` y `skills.ios.guideline.ios-swiftui-expert.prefer-static-member-lookup-blue-vs-color-blue`. Quedan enlazadas en detector textual iOS, extractor, preset heurístico, registry, compiler template, tests de regresión y `skills.lock.json`; el lock actual pasa a `iOS total=320`, `AUTO=65`, `DECLARATIVE=255`, con `ios-swiftui-expert-guidelines` en `total=45`, `AUTO=21`, `DECLARATIVE=24`. Evidencia local: `node --import tsx --test core/facts/detectors/text/ios.test.ts` devuelve `41/41 pass`; `node --import tsx --test core/facts/__tests__/extractHeuristicFacts.test.ts --test-name-pattern 'iOS|SwiftUI|heuristic'` devuelve `100/100 pass`; `node --import tsx --test integrations/config/__tests__/skillsDetectorRegistry.test.ts` devuelve `3/3 pass`; `node --import tsx scripts/compile-skills-lock.ts --check` devuelve `FRESH`.

Último avance operativo: `PUMUKI-INC-128 / RuralGo XCTest brownfield no debe bloquear por Swift Testing obligatorio` corrige el falso bloqueo reportado por RuralGo en `pumuki@6.3.153`: los detectores `skills.ios.prefer-swift-testing`, `skills.ios.no-xctassert` y `skills.ios.no-xctunwrap` dejan de emitir findings sobre suites XCTest-only brownfield (`XCTestCase` sin `import Testing` ni `@Suite/@Test`). La calidad propia de XCTest (`makeSUT()` y `trackForMemoryLeaks()`) sigue en el guard dedicado `governance.skills.ios-test-quality.incomplete`; por tanto Pumuki no fuerza migración a Swift Testing en brownfield, pero puede seguir bloqueando ausencia de patrón de calidad si aplica. Evidencia local: suite dirigida `146/146 pass`; regresión explícita `apps/ios/Tests/Mac/Application/BuyerSessionStoreTokenPersistenceTests.spec.swift` no emite `heuristics.ios.testing.*`; `node --import tsx scripts/compile-skills-lock.ts --check` devuelve `FRESH`; `git diff --check` limpio. Siguiente pieza lógica: commitear, publicar versión útil y repinear primero RuralGo para desbloquear el commit normal sin `--no-verify`.
