...
Último cierre operativo: `[✅] - PUMUKI-INC-081 — PRE_WRITE ya separa blocker duro, warning secundario y tracking accionable en la línea publicada actual` no ha hecho falta patch nuevo: la revalidación actual sobre `pumuki@6.3.109` demuestra que la release ya emite `[pumuki][block-summary] primary=TRACKING_CANONICAL_IN_PROGRESS_INVALID`, separa el warning en `[pumuki][warning-summary] secondary=EVIDENCE_PREWRITE_WORKTREE_WARN` y conserva `active_entries=...` en el mensaje del bloqueo. La cobertura dirigida de `develop` sigue verde vía `integrations/git/__tests__/runPlatformGateOutput.test.ts` y la comprobación runtime se hizo contra el paquete instalado en RuralGo (`/tmp/R_GO__revalidate-inc-080/node_modules/pumuki/integrations/git/runPlatformGateOutput.ts`), así que el cierre es documental/backlog y no requiere nueva release.

Tarea activa única: `[🚧] - PUMUKI-INC-082 — Sincronizar el snapshot superior del backlog con la tabla activa y eliminar deriva documental` tras cerrar `INC-081`, el backlog externo de RuralGo sigue abierto y la prioridad hard pasa al siguiente bug externo vivo de mayor severidad. Mientras `PUMUKI-INC-082` siga reportado en el MD externo, no se permite activar una task interna distinta ni saltar a incidencias de menor severidad.
...
| `RuralGo` | `6.3.109` revalidada en runtime real | `26` incidencias activas | `PUMUKI-INC-080` y `PUMUKI-INC-081` ya cerrados; siguiente bug vivo prioritario `PUMUKI-INC-082`. |
