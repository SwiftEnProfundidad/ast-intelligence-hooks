...
Tarea activa única: `[🚧] - Convergencia release line develop/main — Propagar la línea publicada 6.3.102 a develop antes del cierre de S1` con el backlog externo ya en `0` y `S2.c` cerrado, el siguiente cuello de botella real deja de ser otra micro-slice de smoke y pasa a ser la divergencia de línea entre `develop` (`package.json`/`VERSION` en `6.3.99`) y `main` (`6.3.102` ya publicada); antes de cortar cualquier `release/*` útil para S1 hay que reconciliar esa base para que el siguiente release branch salga de `develop` con la semver y los hotfixes ya convergidos.
