...
Último avance operativo: `[🚧] - PUMUKI-INC-135 / RuralGo PRE_PUSH parity regression` atiende el bug externo reportado desde RuralGo con `pumuki@6.3.190/6.3.191`: PRE_PUSH local bloqueaba por `skills.backend.no-solid-violations` y `skills.frontend.no-solid-violations` agregados no remediables, y el JSON podía conservar bloqueantes stale aunque el gate actual saliera 0. Fix en curso: `*.no-solid-violations` queda advisory por defecto en PRE_PUSH/CI salvo promoción explícita por policy, y `pumuki audit` normaliza findings stale cuando el gate efectivo permite.
