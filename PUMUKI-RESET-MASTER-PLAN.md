...
Último cierre operativo: `[✅] - PUMUKI-INC-084 / PUMUKI-INC-086 — Cerradas por no reproducción en la línea publicada actual` la revalidación real contra `pumuki@6.3.106` en RuralGo y repo fixture confirma que `status`/`doctor` ya no exponen el shape histórico que originó ambas incidencias; además, `develop` mantiene las regresiones dirigidas `readLifecycleStatus alinea governanceNextAction.stage con evidence.snapshot_stage cuando existe evidencia` y `runLifecycleDoctor añade issue canónico y alinea stage cuando governance está bloqueado por evidencia` en verde. El cierre externo queda sincronizado en RuralGo sobre `feature/rgo-1900-01-ios-buyer-core`.

Tarea activa única: `[🚧] - PUMUKI-INC-085 — Hacer inequívoca la semántica de sesión SDD en la línea actual` tras cerrar `084/086/089`, el siguiente bug externo vivo y aún reproducible en RuralGo es `PUMUKI-INC-085`: con `pumuki@6.3.106`, `sdd validate --stage=PRE_WRITE --json` sigue mostrando `session.active=true`, `valid=false` y `remainingSeconds=0` a la vez, así que el runtime continúa mezclando flags incompatibles y obliga a interpretar manualmente si la sesión debe refrescarse, reabrirse o ignorarse.
