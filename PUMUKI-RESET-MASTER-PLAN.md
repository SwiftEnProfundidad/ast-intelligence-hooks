...
|-----------|-----------------|
| Este plan | [🚧] - `PARITY-IOS-001` / cerrar primero cobertura AST por nodos de skills iOS. Estado 2026-05-05: `PUMUKI-INC-128` queda corregido, publicado en `pumuki@6.3.154` y repineado primero en RuralGo; RuralGo ya no bloquea por migración Swift Testing obligatoria en XCTest brownfield, aunque mantiene un bloqueo válido de calidad XCTest (`makeSUT()` / `trackForMemoryLeaks()`) en specs nuevos. Siguiente paso interno: recuperar el stash `wip-parity-ios-swiftui-ast` y cerrar la sub-slice SwiftUI sin mezclar bugs consumer. |
