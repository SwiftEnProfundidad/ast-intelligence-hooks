...
Tarea activa única: `[🚧] - PUMUKI-INC-084 — Hacer canónica la lista de issues en status/doctor` con `083` ya cerrada en la línea publicada y en RuralGo, `086` cerrada en el replay real de `pumuki@6.3.113` y `089` diferida al siguiente slice útil. El objetivo inmediato es que `status` y `doctor` no vuelvan a dejar `issues=[]` cuando el runtime esté en `attention` o `blocked`, para que el contrato sea consumible por automatización antes de abrir el siguiente frente externo.
