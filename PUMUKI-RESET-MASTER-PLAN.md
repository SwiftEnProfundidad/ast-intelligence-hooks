...
|-----------|-----------------|
| Este plan | [🚧] - `PUMUKI-INC-128` / bug externo activo: `pumuki audit --stage=PRE_WRITE --json` no puede forzar `scope.kind=repo` cuando el consumer tiene un staged set acotado. El MD externo de RuralGo declara 1 activo High aunque la referencia sea Stack My Architecture; por contrato anti-bucle esta incidencia congela `PARITY-IOS-SWIFTDATA-001` y cualquier `PUMUKI-2xx` interno hasta tener reproducción, fix, regresión, release útil y actualización del MD externo. Fix local en curso en `bugfix/inc-128-staged-scope`: `runLifecycleAudit` resuelve `scope=staged` para `PRE_WRITE` cuando hay staged soportado, y conserva `repo` si no hay staged soportado. |
...
- Estado de backlog externo tras revalidación 2026-05-12: `R_GO` declara `PUMUKI-INC-128` como único activo High; `SAAS` y `Flux_training` no muestran activos. La prioridad operativa vuelve al bug externo y queda congelado cualquier backlog interno no necesario para cerrarlo.
