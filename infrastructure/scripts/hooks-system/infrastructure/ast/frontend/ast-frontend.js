...
function runFrontendIntelligence(project, findings, platform) {
  project.getSourceFiles().forEach((sf) => {
    ...
    // Prop drilling (heuristic): props forwarded unchanged to multiple children
    sf.getDescendantsOfKind(SyntaxKind.FunctionDeclaration).forEach((fn) => {
      const name = fn.getName();
      ...
      // Count forwards: <Child prop={prop} />
      let forwards = 0;
      body.getDescendantsOfKind(SyntaxKind.JsxAttribute).forEach((attr) => {
        ...
        // Complete props object forwarding
    ...
    // High cyclomatic complexity in components
    sf.getDescendantsOfKind(SyntaxKind.FunctionDeclaration).forEach((fn) => {
      const name = fn.getName();
      if (!name || !/^[A-Z]/.test(name)) return;
      const body = fn.getBody();
    ...
    // React: detect class components (not allowed)
    ...
    // React: using index as key in lists
    sf.getDescendantsOfKind(SyntaxKind.JsxElement).forEach((jsx) => {
      const tag = jsx.getOpeningElement()?.getTagNameNode()?.getText();
      if (tag && /^[A-Z]/.test(tag)) {
        const parent = jsx.getParent();
    ...
    // React: large components that should be composed (>50 lines)
    sf.getDescendantsOfKind(SyntaxKind.FunctionDeclaration).forEach((fn) => {
      const name = fn.getName();
      if (!name || !/^[A-Z]/.test(name)) return;
    ...
    // Accessibility: interactive elements without aria/role
    ...
    // Next.js: unnecessary "use client" (components that could be Server Components)
    ...
    // Performance: heavy imports without dynamic loading
    ...
    // Accessibility/semantics: div/span with onClick without role="button" and tabIndex
    sf.getDescendantsOfKind(SyntaxKind.JsxOpeningElement).forEach((open) => {
      const tag = open.getTagNameNode()?.getText();
    ...
    // React: useEffect without dependency array
    sf.getDescendantsOfKind(SyntaxKind.CallExpression).forEach((call) => {
      const expr = call.getExpression().getText();
      if (expr === "useEffect") {
    ...
    // Assignment to innerHTML in TS/TSX
    ...
    // React: components that should use React.memo
    sf.getDescendantsOfKind(SyntaxKind.FunctionDeclaration).forEach((fn) => {
      const name = fn.getName();
      if (!name || !/^[A-Z]/.test(name)) return;
      ...
      // Component with props but no memoization
    ...
    // React: callbacks without useCallback
    ...
    // React: expensive calculations without useMemo (ONLY in React components, not in services)
    if (isComponent) {
      sf.getDescendantsOfKind(SyntaxKind.CallExpression).forEach((call) => {
        const expr = call.getExpression().getText();
        // Methods that could be expensive: filter, map, reduce, sort, etc.
    ...
    // Backend Integration: loading states
    sf.getDescendantsOfKind(SyntaxKind.CallExpression).forEach((call) => {
      const expr = call.getExpression().getText();
    ...
    // Backend Integration: retry logic
    sf.getDescendantsOfKind(SyntaxKind.CallExpression).forEach((call) => {
      const expr = call.getExpression().getText();
    ...
    // CRITICAL: Catch blocks without explicit type (implicit any in error)
    ...
    // MEDIUM: console.error in catch blocks (should use error state)
    sf.getDescendantsOfKind(SyntaxKind.CallExpression).forEach((call) => {
