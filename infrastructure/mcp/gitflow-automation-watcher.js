...
// Start polling for context changes (every 30s)
setInterval(async () => {
    try {
        // DEBUG: Log al filesystem
        fs.appendFileSync('/tmp/mcp-polling.log', `[${new Date().toISOString()}] Polling tick\n`);
        
        if (!orchestrator.shouldReanalyze()) {
            fs.appendFileSync('/tmp/mcp-polling.log', `[${new Date().toISOString()}] No reanalysis needed (TTL)\n`);
            return;
        }

        const currentContext = await contextEngine.detectContext();
        fs.appendFileSync('/tmp/mcp-polling.log', `[${new Date().toISOString()}] Context: ${JSON.stringify(currentContext.stagedFiles)}\n`);

        if (!contextEngine.hasContextChanged(lastContext)) {
            fs.appendFileSync('/tmp/mcp-polling.log', `[${new Date().toISOString()}] No context change\n`);
            return;
        }

        console.error('[MCP] Context changed, analyzing...');
        fs.appendFileSync('/tmp/mcp-polling.log', `[${new Date().toISOString()}] Context CHANGED, analyzing...\n`);
        const decision = await orchestrator.analyzeContext();
        fs.appendFileSync('/tmp/mcp-polling.log', `[${new Date().toISOString()}] Decision: ${JSON.stringify(decision)}\n`);

        if (decision.confidence >= 70) {
            console.error(`[MCP] Context change detected (${decision.confidence}% confidence): ${decision.platforms.map(p => p.platform).join(', ')}`);
            console.error(`[MCP] Action: ${decision.action}`);

            // 📢 macOS Notification con ruta completa y escape correcto
            const { execSync } = require('child_process');
            const platforms = decision.platforms.map(p => p.platform).join(', ');
            const actionText = decision.confidence >= 90
                ? `AI ejecuto ai-start para ${platforms} (${decision.confidence}% confianza)`
                : `AI preguntara antes de ejecutar para ${platforms} (${decision.confidence}% confianza)`;

            // Escapar comillas y caracteres especiales
            const safeText = actionText.replace(/"/g, '\\"');

            try {
                execSync(`/usr/bin/osascript -e 'display notification "${safeText}" with title "Pumuki AST Intelligence" sound name "Hero"'`, {
                    encoding: 'utf8'
                });
                console.error('[MCP] macOS notification sent ✅');
            } catch (notifError) {
                console.error('[MCP] Failed to send notification:', notifError.message);
            }
        }
