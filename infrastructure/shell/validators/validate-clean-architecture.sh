...
# =============================================================================
# Check Forbidden Directories (ALL platforms)
# =============================================================================
check_forbidden_directories() {
  ...
  if [[ "$dir" =~ (^|/)lib(/|$) ]]; then
    ...
    echo -e "   File: $file"
    echo -e "   Fix: Use ${GREEN}infrastructure/${NC} or ${GREEN}presentation/${NC}"
    ((VIOLATIONS++))
    return 1
  fi
  ...
  if [[ "$dir" =~ (^|/)utils(/|$) ]]; then
    ...
    echo -e "   File: $file"
    echo -e "   Fix: Use ${GREEN}application/${NC} or ${GREEN}domain/${NC}"
    ((VIOLATIONS++))
    return 1
  fi
  ...
  if [[ "$dir" =~ (^|/)helpers(/|$) ]]; then
    ...
    echo -e "   File: $file"
    echo -e "   Fix: Use ${GREEN}application/use-cases/${NC}"
    ((VIOLATIONS++))
    return 1
  fi
  ...
  if [[ "$dir" =~ (^|/)docs(/|$) ]]; then
    ...
    echo -e "   File: $file"
    echo -e "   Fix: Documentation should be:"
    echo -e "     - README.md in root"
    echo -e "     - README.md in each module directory"
    echo -e "     - NOT in separate docs/ folder"
    ((VIOLATIONS++))
    return 1
  fi
...
validate_frontend_file() {
  ...
  if [[ "$filename" =~ ^use[A-Z].*\.(ts|tsx)$ ]]; then
    if [[ ! "$dir" =~ presentation/hooks$ ]] && [[ ! "$dir" =~ app/.*/hooks$ ]]; then
      ...
      echo -e "   File: $file"
      echo -e "   Rule: Custom hooks → ${GREEN}presentation/hooks/${NC} or ${GREEN}app/**/hooks/${NC}"
      echo -e "   Found in: ${RED}$dir${NC}"
      echo -e "   Fix: Move to presentation/hooks/ or keep in app feature directory"
      ((VIOLATIONS++))
      return 1
    fi
  fi
