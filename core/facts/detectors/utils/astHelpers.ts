export const isObject = (value: unknown): value is Record<string, string | number | boolean | bigint | symbol | null | Date | object> => {
  return typeof value === 'object' && value !== null;
};

export const hasNode = (
  node: unknown,
  predicate: (value: Record<string, string | number | boolean | bigint | symbol | null | Date | object>) => boolean
): boolean => {
  if (!isObject(node)) {
    return false;
  }
...
const extractStartLine = (node: Record<string, string | number | boolean | bigint | symbol | null | Date | object>): number | null => {
  const loc = node.loc;
  if (!isObject(loc)) {
    return null;
  }
