...
HOOKS_CONFIG="${ADAPTER_HOOKS_CONFIG}"

if [ ! -f "${HOOKS_CONFIG}" ] && [ -f "${WINDSURF_HOOKS_CONFIG}" ]; then
  HOOKS_CONFIG="${WINDSURF_HOOKS_CONFIG}"
fi

if [ ! -f "${HOOKS_CONFIG}" ]; then
  echo "[pumuki:cascade-hooks] missing adapter config: ${ADAPTER_HOOKS_CONFIG}" >&2
  if [ "${ADAPTER_HOOKS_CONFIG}" != "${WINDSURF_HOOKS_CONFIG}" ]; then
