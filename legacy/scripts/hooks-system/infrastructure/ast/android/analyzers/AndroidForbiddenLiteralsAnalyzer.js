...
class AndroidForbiddenLiteralsAnalyzer {
    ...
    analyzeStringLiterals(stringLiterals, sf, findings, pushFinding) {
        stringLiterals.forEach((literal) => {
            ...
            if (isStatusContext && FORBIDDEN_STATUS_LITERALS.includes(value)) {
                pushFinding(
                    'android.types.forbidden_status_literal',
                    'high',
                    literal,
                    sf,
    ...
    analyzeNullishCoalescing(nullishCoalescing, sf, findings, pushFinding) {
        nullishCoalescing.forEach((expr) => {
            ...
            if (hasNull || hasUndefined) {
                pushFinding(
                    'android.types.forbidden_nullish_coalescing',
                    'critical',
                    expr,
                    sf,
                    `Nullish coalescing with null/undefined detected. Use optional properties (name?: String) instead of nullable types (name: String?). Eliminate null/undefined from codebase.`,
                    findings
                );
            }
        });
    ...
    analyzeConditionalExpressions(conditionalExpressions, sf, findings, pushFinding) {
        conditionalExpressions.forEach((expr) => {
            ...
            if (hasNull || hasUndefined) {
                pushFinding(
                    'android.types.forbidden_ternary_null_undefined',
                    'critical',
                    expr,
                    sf,
                    `Ternary operator with null detected. Use optional properties (prop: String?) and conditional spread pattern. Only include properties when they have values. Use data class with optional parameters.`,
                    findings
                );
            }
        });
    ...
    analyzeObjectLiterals(objectLiteralExpressions, sf, findings, pushFinding) {
        objectLiteralExpressions.forEach((obj) => {
            ...
            const isInTestFile = /\.(spec|test)\.(kt|java)$/i.test(sf.getFilePath());
            const hasNullAssignment = /:\s*null/.test(fullLine);

            if (isInTestFile) return;

            if (hasNullAssignment) {
                pushFinding(
                    'android.types.forbidden_null_assignment',
                    'critical',
                    obj,
                    sf,
                    `Direct null assignment in object detected. Use optional properties and conditional spread pattern. Only include properties when they have values. Use data class with optional parameters (prop: String?).`,
                    findings
                );
            }
        });
...
module.exports = { AndroidForbiddenLiteralsAnalyzer };
