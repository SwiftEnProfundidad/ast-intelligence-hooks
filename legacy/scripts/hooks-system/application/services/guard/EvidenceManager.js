...
EvidenceManager.prototype.startPolling = function startPolling(onStale, onRefresh) {
...
EvidenceManager.prototype.triggerStaleAlert = function triggerStaleAlert(source, ageMs) {
    const now = Date.now();
...
EvidenceManager.prototype.attemptAutoRefresh = async function attemptAutoRefresh(reason = 'manual') {
...
EvidenceManager.prototype.runDirectEvidenceRefresh = async function runDirectEvidenceRefresh(_reason) {
    const updateScriptCandidates = [
        path.join(process.cwd(), 'scripts/hooks-system/bin/update-evidence.sh'),
...
EvidenceManager.prototype.updateUserActivity = function updateUserActivity() {
    this.lastUserActivityAt = Date.now();
};

EvidenceManager.prototype.readEvidenceTimestamp = function readEvidenceTimestamp() {
    try {
...
EvidenceManager.prototype.evaluateEvidenceAge = function evaluateEvidenceAge(source = 'manual', notifyFresh = false) {
    const now = Date.now();
...
EvidenceManager.prototype.stopPolling = function stopPolling() {
    if (this.pollTimer) {
        clearInterval(this.pollTimer);
        this.pollTimer = null;
    }
