...
- 🚧 Continuar siguiente lote post-release: desbloquear validación enterprise sobre baseline normalizada del mock (matriz + checks de stage/evidence). Bloqueo actual reproducido en entorno temporal: `npm run pumuki:matrix` falla en `scenario:clean` con `pre-commit=0`, `pre-push=1`, `ci=1` por reglas metodológicas `methodology.tdd.backend-domain-change-requires-tests` y `methodology.bdd.backend-application-change-requires-spec` (logs: `/tmp/pumuki-clean-pre-push.log`, `/tmp/pumuki-clean-ci.log`).
