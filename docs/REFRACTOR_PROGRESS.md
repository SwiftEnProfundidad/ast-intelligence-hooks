...
- ✅ Next local refactor batch prepared: move to `fs.promises.unlink` semantic heuristic with deterministic tests/docs.
...
- 🚧 `P9-1` Execute next advanced AST semantic heuristic in local-only mode: `fs.promises.unlink` detection (external billing path omitted).
