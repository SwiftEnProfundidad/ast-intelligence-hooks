...
- `GET /ai-evidence/summary`
  - `200` with compact deterministic summary (including `snapshot.has_findings`, `snapshot.findings_files_count`, `snapshot.findings_with_lines_count`, `snapshot.findings_without_lines_count`, `snapshot.severity_counts`, `snapshot.findings_by_platform`, `snapshot.highest_severity`, `snapshot.blocking_findings_count`, `ledger_count`, `ledger_files_count`, `ledger_by_platform`, `rulesets_by_platform`, `rulesets_fingerprint`, `platform_confidence_counts`, `suppressed_findings_count`, `tracked_platforms_count`, `detected_platforms_count`, and `non_detected_platforms_count`)
  - `404` when missing or invalid
