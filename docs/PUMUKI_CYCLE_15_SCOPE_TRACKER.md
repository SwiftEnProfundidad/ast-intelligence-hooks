# PUMUKI Cycle 15 — Scope Tracker

Seguimiento simple del ciclo 15 para mantener ejecución finita, sin bucles y con visibilidad completa.
...
- ✅ C15-T1: Abrir tracker del ciclo con alcance único y criterio verificable.
- ✅ C15-T2: Ejecutar preflight del mock (`branch/upstream/working tree`) y confirmar baseline limpia.
- ✅ C15-T3: Ejecutar comando único del criterio y capturar salida real.
- ✅ C15-T4: Ejecutar validación final del alcance (re-ejecución + verificación de artefacto).
- ✅ C15-T5: Cerrar ciclo (actualizar trackers + commit/push).

## Evidencia de validación final (C15-T4)
...
- Re-ejecución final del comando de criterio:
  ...
  - `MATRIX_EXIT=0`.
- Artefacto final:
  - `run_id=pumuki-matrix-20260219T234255Z-66395`
  - `package_spec=pumuki@latest`
  - `final_verdict=PASS`
  - escenarios con patrón esperado (`clean=0/0/0`, `violations=1/1/1`, `mixed=1/1/1`).

## Cierre de ciclo (C15-T5)

- Trackers de ciclo y global actualizados sin tareas abiertas de `C15`.
