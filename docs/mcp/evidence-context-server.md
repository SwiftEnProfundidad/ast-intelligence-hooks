...
- `/status.evidence.exists` is always a boolean (`true|false`, never `null`).
- `/status.evidence.present` remains for compatibility and mirrors the existence state.
