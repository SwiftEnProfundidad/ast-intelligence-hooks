...
- `GET /ai-evidence/summary`: compact deterministic summary (`stage/outcome/counts/severity_counts/findings_by_platform/highest_severity/rulesets_by_platform/detected platforms`)
...
- `GET /status`: lightweight summary (`present/valid/version/stage/outcome/counts/severity_counts/findings_by_platform/highest_severity/rulesets_by_platform`) plus `context_api` capabilities (`endpoints`, supported filters, deterministic pagination bounds)
