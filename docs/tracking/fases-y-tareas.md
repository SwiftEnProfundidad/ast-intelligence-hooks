...
- ✅ Ultima task cerrada: `P2.F1.T4` Cerrar la paridad final `source-bin` / paquete instalado y sanear los wrappers restantes del consumer (`menu avanzado`, `export`, ayudas de soporte y DX visible de hooks) para que consuman la misma verdad canónica del core sin narrativa heredada ni hints inválidos.
- Task activa: `P2.F2.T1` Revalidar y reducir la deuda viva de `Core / Policy / Experimental` que sigue abierta en `Fixture Repo 01`, priorizando `lifecycle CLI`, `bootstrap hygiene`, `doctor/policy fallback`, `CI/session bootstrap` y los falsos positivos que todavía sobreviven al reset, para decidir qué se estabiliza en Core, qué se mueve a `Policy Packs` y qué se congela definitivamente en `Experimental`.
...
- ✅ `P2.F1.T4` Cerrar la paridad final `source-bin` / paquete instalado y sanear los wrappers restantes del consumer (`menu avanzado`, `export`, ayudas de soporte y DX visible de hooks) para que consuman la misma verdad canónica del core, no reintroduzcan findings legacy y degraden limpio cuando el paquete instalado vaya por detrás del source o falten capacidades opcionales del consumer. El slice queda cerrado con la convergencia del menú avanzado al `runtime summary` canónico, la corrección del `workflow lint` para no interpretar `exit_code != 0` y salida vacía como estado limpio, `support ticket draft` y `support bundle` ya evidence-driven, `startup triage` ejecutando scripts desde el paquete Pumuki, `pumuki-pre-commit` emitiendo progreso visible al arrancar y el render legacy usando el stage real (`PRE_PUSH` / `CI`) en el resumen final.
- 🚧 `P2.F2.T1` Revalidar y reducir la deuda viva de `Core / Policy / Experimental` que sigue abierta en `Fixture Repo 01`, priorizando `lifecycle CLI`, `bootstrap hygiene`, `doctor/policy fallback`, `CI/session bootstrap`, falsos positivos de heurísticas y fricciones de `evidence` para decidir qué se estabiliza en Core, qué se promueve a `Policy Packs` y qué queda congelado en `Experimental`. Este slice ya nace con una frontera clara: no volver a tocar `UX / Reporting` salvo regresión directa; el foco pasa a lo que todavía hace ruido estructural aunque los wrappers ya estén alineados.
