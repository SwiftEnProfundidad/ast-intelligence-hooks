...
- ✅ **Paquete 6.3.72 (repo):** `package.json` → `files` + `CHANGELOG` del producto + smoke test alineados; curso **stack-my-architecture-pumuki** repineado a **jsDelivr `pumuki@6.3.72`** y textos de bienvenida/changelog del curso coherentes con tarball. **Commit en `release/6.3.65`:** **`0fd34fa`** — mensaje `release: pumuki 6.3.72 — lifecycle audit, surface smoke, postinstall MCP` (77 ficheros). **Hook PRE_COMMIT:** el umbral por defecto (`pending_changes` > **24**) bloqueaba el bundle; commit ejecutado con **`PUMUKI_PREWRITE_WORKTREE_BLOCK_THRESHOLD=120`** y **`PUMUKI_PREWRITE_WORKTREE_WARN_THRESHOLD=80`** (documentado en `docs/product/CONFIGURATION.md`), **sin** `--no-verify`. **Siguiente paso operativo:** `git push` (cuando indiques) + **`npm publish`** **6.3.72** + validar jsDelivr + repin consumidores.
