...
- ✅ **Barra local + env global notificaciones:** `framework-menu-system-notifications-lib.test.ts` con **`env` sin `PUMUKI_DISABLE_SYSTEM_NOTIFICATIONS`** donde el caso exige entrega/fallback; con **`PUMUKI_DISABLE_SYSTEM_NOTIFICATIONS=1`** en el shell, **`npm run validation:local-merge-bar`** **exit 0** (~**132 s**). En **release** commit **`90a105c`**; en **`develop`** PR [**#750**](https://github.com/SwiftEnProfundidad/ast-intelligence-hooks/pull/750) (**`8007722`**) — mismo criterio, integrado en **release/6.3.65** vía merge `develop`.
- ✅ **Tests MCP v2.1 + política CI (`develop` → esta rama):** **#748** (`34b98b8` en `release/6.3.65`, squash **`cfbcc9b`** en `develop`); **#749** política sin cuota Actions; **#750** aislamiento de tests de notificaciones. Merges aplicados **sin depender de CI remoto** (política **Estado actual**).
...
3. 🚧 **RuralGO (producto) — ejecución release `develop` → `main`**: **foco actual**. Charter (PRs doc **#1627–#1631**; [**#1631**](https://github.com/SwiftEnProfundidad/R_GO/pull/1631) **MERGED**). **Alineación doc SHA en charter:** [**#1632**](https://github.com/SwiftEnProfundidad/R_GO/pull/1632) **MERGED** a `develop` (**2026-04-12**; charter ↔ tip previo a ese merge **`a93b868c9…`**). **Rama [`release/1.0.1`](https://github.com/SwiftEnProfundidad/R_GO/tree/release/1.0.1):** tip remoto **`54aee9413`** — incluye **FF** a `develop` (**`a2367ba03…`**) + **merge `main`** (conflictos resueltos en `docs/README.md`, `docs/RURALGO_SEGUIMIENTO.md`, `docs/technical/06-ci-cd/GIT_HOOKS_SETUP.md`); [**#1629**](https://github.com/SwiftEnProfundidad/R_GO/pull/1629) vuelve **MERGEABLE** (GitHub puede mostrar **UNSTABLE** por checks/cuota). Comentarios: [sync develop](https://github.com/SwiftEnProfundidad/R_GO/pull/1629#issuecomment-4230626874), [resolución conflictos](https://github.com/SwiftEnProfundidad/R_GO/pull/1629#issuecomment-4230637995), [verificación sin delta develop](https://github.com/SwiftEnProfundidad/R_GO/pull/1629#issuecomment-4230645520). Si `develop` avanza otra vez, valorar **FF** adicional antes del merge final a `main`. [**PR #1629** — **borrador** → `main`](https://github.com/SwiftEnProfundidad/R_GO/pull/1629) — checklist charter **sigue abierto**; **no** fusionar a `main` sin owner nominal + ítems marcados con evidencia (`git rev-parse origin/develop` tras `git pull` al cerrar checklist). **Semver `1.0.1`:** validar con producto.
