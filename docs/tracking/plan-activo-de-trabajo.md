...
- ⏳ **Repin consumidores a 6.3.72** (RuralGO / SAAS / Flux si aún en **6.3.71**): **npm `latest`** ya es **6.3.72**; el código Git del release está en **`origin/release/6.3.65`**. Contenido relevante para consumidores: **`gate.blocked`** (banner + modal); **postinstall** — `pumuki install --with-mcp --agent=repo` para **`.pumuki/adapter.json`** (opt-out `PUMUKI_POSTINSTALL_SKIP_MCP=1`). Tras repin: `pumuki doctor --json` / smoke instalado opcional.
