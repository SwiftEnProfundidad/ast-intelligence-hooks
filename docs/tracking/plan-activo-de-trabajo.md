...
- ⏳ **Repin consumidores a 6.3.72** (RuralGO / SAAS / Flux si aún en **6.3.71**): **npm `latest`** ya es **6.3.72**; el código Git del release está en **`origin/release/6.3.65`**. **Comprobación local (solo lectura `package.json` en esta máquina):** **`R_GO/package.json`** → `"pumuki": "6.3.71"`; **`SAAS:APP_SUPERMERCADOS/package.json`** (raíz) → **6.3.71**; **`Flux_training/package.json`** → **6.3.71** → **pendiente bump + lockfile** en cada repo consumidor (cambios **fuera** de `ast-intelligence-hooks`). Contenido relevante post-repin: **`gate.blocked`**; **postinstall** — `pumuki install --with-mcp --agent=repo` para **`.pumuki/adapter.json`** (`PUMUKI_POSTINSTALL_SKIP_MCP=1` opt-out). Tras repin: `pumuki doctor --json` / smoke instalado opcional.
...
- ⏳ **R_GO** manifest local aún **6.3.71** (ver línea repin arriba); tras repin **6.3.72**: `pumuki doctor --json` / `lifecycleInstalled` sin drift.
- ⏳ **SAAS** raíz local aún **6.3.71**; tras repin **6.3.72**: si la ruta del repo lleva `:` en macOS, workaround `node ./node_modules/pumuki/bin/pumuki.js` donde haya `pathExecutionHazard`.
- ⏳ **Flux_training** local aún **6.3.71** (`pnpm-lock.yaml` a actualizar con repin); tras bump: `pnpm exec pumuki install` si drift de lifecycle.
