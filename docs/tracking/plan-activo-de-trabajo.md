...
- ✅ **Tests MCP v2.1:** en `release/6.3.65` (`34b98b8`); en **`develop`** squash **`cfbcc9b`** vía PR https://github.com/SwiftEnProfundidad/ast-intelligence-hooks/pull/748. Merge **sin depender de CI remoto** (política **Estado actual**).
- ✅ **Barra local + notificaciones (env heredado):** `framework-menu-system-notifications-lib.test.ts` usa `env` sin `PUMUKI_DISABLE_SYSTEM_NOTIFICATIONS` donde el caso exige stderr/macOS; con **`PUMUKI_DISABLE_SYSTEM_NOTIFICATIONS=1`** en el shell, **`npm run validation:local-merge-bar`** **exit 0** (~**132 s**). Integrado en **`develop`** commit **`0ee9a7c`** (mismo parche que `90a105c` en `release/6.3.65`).
- ✅ **Rama `release/6.3.65` ↔ `develop`:** merge **`72ad9bf`** en **`origin/release/6.3.65`** incorpora el tip actual de **`develop`** (incl. **#748–#750**). **Decisión:** no abrir merge inverso masivo **release → `develop`** (riesgo de conflictos en tracking); el espejo operativo de **6.3.72** vive en **release**; **`develop`** sigue siendo línea de integración de producto.
