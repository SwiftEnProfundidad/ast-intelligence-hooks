...
- Task activa (`🚧`): `P2.F1.T4` Cerrar la paridad final `source-bin` / paquete instalado y sanear los wrappers restantes del consumer (`menu avanzado`, `export` y ayudas de soporte) para que consuman la misma verdad canónica del core, no reintroduzcan findings legacy y degraden limpio cuando el paquete instalado vaya por detrás del source o falten capacidades opcionales del consumer. El `support ticket draft` ya pasó a ser evidence-driven, `startup triage` ya ejecuta scripts desde el paquete y no desde el consumer, y el foco restante queda en los wrappers que todavía pueden divergir del core.
