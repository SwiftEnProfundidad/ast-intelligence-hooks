...
export type PreWriteOpenSpecBootstrapTrace = {
  enabled: boolean;
  attempted: boolean;
  status: 'SKIPPED' | 'OK' | 'FAILED';
  actions: string[];
...
export type LifecycleCliDependencies = {
...
export const resolvePreWriteNextAction = (params: {
  sdd: SddEvaluateResult;
...
export const resolvePreWriteBlockedRemediation = (params: {
  causeCode: string;
...
export const buildPreWriteValidationPanel = (params: {
  sdd: SddEvaluateResult;
  aiGate: ReturnType<typeof evaluateAiGate>;
  automation: PreWriteAutomationTrace;
}): string => {
...
export const resolveSddDecisionLocation = (result: SddEvaluateResult) => {
...
export const resolveAiGateViolationLocation = (code: string) => {
...
export const buildPreWriteValidationEnvelope = (
...
export const runRawPreWriteAiGateCheck = (params: {
  repoRoot: string;
...
export const runLifecycleCli = async (
  ...
  try {
    ...
    switch (parsed.command) {
      ...
      case 'sdd': {
        const { runSddCommand } = await import('./cliSdd');
        return await runSddCommand(parsed, activeDependencies);
      }
      case 'adapter': {
