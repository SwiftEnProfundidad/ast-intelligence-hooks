...
test('upsertPumukiManagedBlock antepone Pumuki antes de hook pre-commit con exec (código muerto si fuera al final)', () => {
  const framework = [
    '#!/usr/bin/env bash',
  ...
  const pumukiIdx = updated.indexOf('PUMUKI MANAGED START');
  const execIdx = updated.indexOf('exec "$INSTALL_PYTHON"');
  assert.ok(pumukiIdx !== -1);
  assert.ok(execIdx !== -1);
  assert.ok(pumukiIdx < execIdx);
});

test('upsertPumukiManagedBlock antepone Pumuki antes de hook pre-push con exec (código muerto si fuera al final)', () => {
  const framework = [
    '#!/usr/bin/env bash',
    '# File generated by pre-commit: https://pre-commit.com',
  ...
  assert.ok(execIdx !== -1);
  assert.ok(pumukiIdx < execIdx);
  assert.match(updated, /pumuki-pre-push/);
});
